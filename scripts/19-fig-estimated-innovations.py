...
with saved_figure('figures-tables/innovation_series.pdf', nrows=1, ncols=3) as (fig,ax):
     for (axi, plotvar) in zip(ax.reshape(-1), ['v_epsxi', 'v_epsi', 'v_epsy']):
         axi.fill_between(index, q05[plotvar], q95[plotvar], alpha=0.3)
         axi.plot(index, mu[plotvar].values)

     fig.set_size_inches(14,3);
     ax[0].set_title(r'$\epsilon_{\xi,t}$', fontsize=19, usetex=True)
     ax[1].set_title(r'$\epsilon_{i^*,t}$', fontsize=19, usetex=True)
     ax[2].set_title(r'$\epsilon_{y^*,t}$', fontsize=19, usetex=True)
     fig.tight_layout()
